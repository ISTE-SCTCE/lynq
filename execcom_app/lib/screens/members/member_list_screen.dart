...
class _MemberListScreenState extends State<MemberListScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadUsers() async {
    try {
      final data = await Supabase.instance.client.from('users').select().order('name');
      final now = DateTime.now();
      _allUsers = (data as List)
          .map((e) => UserModel.fromJson(e))
          .map((u) {
            // Auto expiry logic (optimistic update to inactive if expired)
            if (u.expiryDate != null && u.expiryDate!.isBefore(now) && !u.role.contains('expired')) {
              // Note: You might want a server cron job for actual role downgrade.
              // Here we just mark them visually or handle them gracefully.
            }
            return u;
          })
          .toList();
  ...
  List<UserModel> get _filteredUsers {
    ...
    final now = DateTime.now();
    if (_filter == 'expired') {
      list = list.where((u) => u.expiryDate != null && u.expiryDate!.isBefore(now) || u.role.contains('expired')).toList();
    } else if (_filter != 'all') {
  ...
  Color _roleColor(String role) => switch (role) {
    'chairman' => Colors.amber,
    'vice_chairman' => Colors.orange,
    'core_execcom' => AppTheme.secondary,
    'execcom' => Colors.teal,
    'member' => Colors.grey,
    _ => Colors.grey,
  };
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                _filterChip('Core Execcom', 'core_execcom'),
                _filterChip('Execcom', 'execcom'),
