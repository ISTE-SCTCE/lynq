...
class _ReportUploadScreenState extends State<ReportUploadScreen> {
  ...
  List<AppRole> _selectedVisibility = [];
  ...
  void initState() {
    ...
    if (widget.existingReport != null) {
      ...
      final visibilityRaw = widget.existingReport!['visibility'];
      if (visibilityRaw is List) {
        _selectedVisibility = visibilityRaw.map((v) => AppRole.fromString(v.toString())).toList();
      }
    }
  }
    ...
    try {
      ...
      if (widget.existingReport == null) {
        await Supabase.instance.client.from('event_reports').insert({
          ...
          'visibility': _selectedVisibility.map((e) => e.toDbString()).toList(),
        });
      } else {
        ...
        child: GlassCard(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text('Visibility (Who can view this report)', style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: Colors.grey[400])),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: AppRole.values.map((role) {
                  // Chairman implicitly has access, but we let them select it if they want to be explicit.
                  final isSelected = _selectedVisibility.contains(role);
                  return FilterChip(
                    label: Text(role.label, style: TextStyle(color: isSelected ? Colors.white : Colors.black87)),
                    selected: isSelected,
                    ...
                    onSelected: (selected) {
                      setState(() {
                        if (selected) {
                          _selectedVisibility.add(role);
                        } else {
                          _selectedVisibility.remove(role);
                        }
                      });
                    },
                  );
