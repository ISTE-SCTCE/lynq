...
class _BudgetOverviewScreenState extends State<BudgetOverviewScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadBudget() async {
    try {
      ...
      _myFolderIds = (membershipData as List).map((m) => m['execom_id'] as int).toList();
      ...
      if (perms != null && !perms.isAtLeastTier2) {
        query = query.inFilter('execom_id', _myFolderIds);
      }
      ...
      if (canViewTotal) {
        ...
        final ledgerData = await Supabase.instance.client
            .from('financial_ledger')
            .select()
            .inFilter('execom_id', _myFolderIds)
            .order('transaction_date', ascending: false);
      ...
      if (!canViewTotal && _myFolderIds.isNotEmpty) {
        final allocationData = await Supabase.instance.client
            ...
            .filter('execom_id', 'in', '(${_myFolderIds.join(',')})');
  ...
  Future<void> _confirmTransaction() async {
    ...
    try {
      ...
      await Supabase.instance.client.from('financial_ledger').insert({
        ...
        'execom_id': widget.myFolderIds.isNotEmpty ? widget.myFolderIds.first : null,
