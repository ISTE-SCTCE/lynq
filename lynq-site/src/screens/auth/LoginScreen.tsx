...
export const LoginScreen: React.FC = () => {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    if (hasError) {
      setError('Please resolve the field errors below.');
      return;
    }
    ...
    try {
      ...
      setError(msg);
      // Give both inputs a subtle shake on failure
      setEmailError(' ');
      setPasswordError(' ');
    } finally {
      setIsLoading(false);
            ...
            <AnimatePresence mode="wait">
              {error && (
                <motion.div 
                  ...
                  className="login-error-container flex-center"
                  style={{ overflow: 'hidden', width: '100%' }}
                >
                  <AlertCircle size={16} className="error-icon" style={{ flexShrink: 0 }} />
