...
export const BudgetOverviewScreen: React.FC = () => {
  ...
  const loadBudgetData = async () => {
    ...
    try {
      ...
      const fIds = (memberData || []).map((m: any) => m.folder_id);
      setMyFolderIds(fIds);
  ...
  return (
    <div className="budget-overview-container">
      <header className="page-header">
        ...
        <h2 className="page-title">{isAtLeastTier2 ? 'Budget Management' : 'Team Budget'}</h2>
              ...
              <GlassCard className="budget-metrics-card" padding="24px">
                ...
                <div className="summary-split-row">
                  <div className="split-col">
                    <span className="split-label">{isAtLeastTier2 ? 'Total Income' : 'Team Allocation'}</span>
        ...
        <div className="modal-overlay">
          <GlassCard className="modal-card" padding="24px">
            ...
            <form onSubmit={handleNewTransaction}>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label className="modal-input-label">Team Scoped association</label>
                <select
                  value={transFolder}
                ...
                >
                  {myFolderIds.map((fid) => (
                    <option key={fid} value={fid}>Team #{fid}</option>
                  ))}
                </select>
              </div>
