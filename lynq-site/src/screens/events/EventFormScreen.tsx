  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: newEvent, error } = await supabase.from('events').insert({
        ...
        certificate_image_url: templateUrl.trim() || null,
    ...
    <div className="event-form-container">
      ...
      <form onSubmit={handleSubmit} className="event-form" style={{ marginBottom: '40px' }}>
        <GlassCard className="form-fields-card" padding="24px">
          ...
          <div style={{ marginBottom: '16px' }}>
            <label className="form-input-label">Google Slides Certificate Template Link</label>
            <input
              type="text"
              value={templateUrl}
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'block', marginTop: '6px' }}>
              Contains tags like <code>{`{{Student_name}}`}</code>, <code>{`{{chair_name}}`}</code>, <code>{`{{coord_name}}`}</code>
            </span>
          </div>
