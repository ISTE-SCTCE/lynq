...
export const FolderListScreen: React.FC = () => {
  ...
  const fetchFolders = async () => {
    ...
    try {
      ...
      if (!permissions.isAtLeastTier2) {
        ...
        const folderIds = (memberFolders || []).map((m: any) => m.folder_id);
  ...
  const handleDeleteFolder = async (id: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm('Are you sure you want to delete this team? All members and linked data will be removed.')) return;
  ...
  return (
    <div className="folder-list-container">
      <header className="page-header">
        ...
        <h2 className="page-title">Active Teams</h2>
      ...
      {isLoading ? (
        <div className="folders-loading">Loading active teams...</div>
      ) : folders.length === 0 ? (
        <div className="folders-empty">No teams available.</div>
      ) : (
          ...
          >
            ...
            >
              <GlassCard className="modal-card" padding="24px">
                ...
                <input
                  type="text"
                  placeholder="Team Name (e.g. IEEE, CSI)"
                  value={newFolderName}
